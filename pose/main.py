    ...
    def run(self):
        ...
        for i in range(self.args.num_total_iterations):
            ...
            print(
                ...
                % (i + 1, args.num_total_iterations, np.mean(all_iterations_accuracy_meter_top_all),
                   np.mean(all_iterations_accuracy_meter_top_body), np.mean(all_iterations_accuracy_meter_top_face),
    ...
    def cross_validation(self, num_splits):
        ...
        for n in range(num_splits):
            ...
            print(
                ...
                % (n + 1, num_splits, np.mean(cross_val_accuracy_meter_top_all),
        ...
        for self.current_epoch in range(0, self.args.epochs):
            ...
            if self.current_epoch == self.args.epochs - 1:
                print(
                    ...
                    % (self.current_epoch, self.args.epochs, train_loss, val_loss, train_acc, val_top_all, val_top_body,
                       val_top_face, lr))
                ...
                if self.current_epoch == self.args.epochs - 1:
                    ...
                    np.save(
                        "saved_scores/%s_out_%s_%s_%d" % (self.args.exp_name,
                                                          self.args.db, self.current_split, self.current_iteration),
                        out)
                    np.save("saved_scores/%s_y_%s_%s_%d" % (
                        self.args.exp_name, self.args.db, self.current_split, self.current_iteration), y)
                    ...
                    if self.args.split_branches:
                        ...
                        np.save(
                            "saved_scores/%s_out_face_%s_%s_%d" % (self.args.exp_name,
                                                                   self.args.db, self.current_split,
                                                                   self.current_iteration), out_face)
                        np.save("saved_scores/%s_y_face_%s_%s_%d" % (
                            self.args.exp_name, self.args.db, self.current_split, self.current_iteration), y_face)

                        np.save(
                            "saved_scores/%s_out_body_%s_%s_%d" % (self.args.exp_name,
                                                                   self.args.db, self.current_split,
                                                                   self.current_iteration), out_body)
                        np.save("saved_scores/%s_y_body_%s_%s_%d" % (
                            self.args.exp_name, self.args.db, self.current_split, self.current_iteration), y_body)
                    ...
                    if (self.args.do_fusion):
                        ...
                        np.save(
                            "saved_scores/%s_out_fusion_%s_%s_%d" % (self.args.exp_name,
                                                                     self.args.db, self.current_split,
                                                                     self.current_iteration), out_fusion)
